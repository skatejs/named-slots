(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(['module', 'exports', './util/each', './util/can-patch-native-accessors', './util/get-property-descriptor', 'debounce', './util/get-escaped-text-content', './util/get-comment-node-outer-html', './version', 'weakmap', 'custom-event-polyfill'], factory);
  } else if (typeof exports !== "undefined") {
    factory(module, exports, require('./util/each'), require('./util/can-patch-native-accessors'), require('./util/get-property-descriptor'), require('debounce'), require('./util/get-escaped-text-content'), require('./util/get-comment-node-outer-html'), require('./version'), require('weakmap'), require('custom-event-polyfill'));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod, mod.exports, global.each, global.canPatchNativeAccessors, global.getPropertyDescriptor, global.debounce, global.getEscapedTextContent, global.getCommentNodeOuterHtml, global.version, global.weakmap, global.customEventPolyfill);
  ...
  function slotNodeIntoSlot(slot, node, insertBefore) {
    ...
    if (shouldGoIntoContentMode) {
      forEach.call(slot.__childNodes, function (node) {
  ...
  function appendChildOrInsertBefore(host, newNode, refNode) {
    ...
    if (!_canPatchNativeAccessors2.default && !host.childNodes.push) {
      staticProp(host, 'childNodes', [].concat(_toConsumableArray(host.childNodes)));
    }
